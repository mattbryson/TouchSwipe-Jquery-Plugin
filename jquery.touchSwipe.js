	...
	$.fn.swipe = function(options) 
	{
		...
		return this.each(function() 
		{
			...
			*/
			...
			{
                var evt = SUPPORTS_TOUCH ? event.touches[0] : event; 
				phase = PHASE_START;
				...
				if (fingerCount == defaults.fingers || !SUPPORTS_TOUCH) 
				{
			...
			*/
			function touchMove(event) 
			{
				...
				if ( fingerCount == defaults.fingers || !SUPPORTS_TOUCH) 
				{
					...
					if (defaults.swipeStatus)
						triggerHandler(event, phase, direction, distance, duration);
					
					//If we trigger whilst dragging, not on touch end, then calculate now...
					if (!defaults.triggerOnTouchEnd)
					{
						// if the user swiped more than the minimum length, perform the appropriate action
						if ( distance >= defaults.threshold  && validateSwipeTime()) 
						{
							phase = PHASE_END;
							triggerHandler(event, phase);
			...
			*/
			function touchEnd(event) 
			{
				...
				if (defaults.triggerOnTouchEnd)
				{
					...
					if ( (fingerCount == defaults.fingers  || !SUPPORTS_TOUCH) && end.x != 0 ) 
					{
						// if the user swiped more than the minimum length, perform the appropriate action
						if ( distance >= defaults.threshold && validateSwipeTime())
						{
			...
			*/
			function triggerHandler(event, phase) 
			{
				//update status
				if (defaults.swipeStatus)
					defaults.swipeStatus.call($this,event, phase, direction || null, distance || 0, duration || 0);
				
				
				if (phase == PHASE_CANCEL)
				{
					if (defaults.click && (fingerCount==1 || !SUPPORTS_TOUCH) && (isNaN(distance) || distance==0))
					...
					if (defaults.swipe)
				{
						...
						defaults.swipe.call($this,event, direction, distance, duration);
					...
					switch(direction)
					{
						case LEFT :
							if (defaults.swipeLeft)
								defaults.swipeLeft.call($this,event, direction, distance, duration);
							break;
						
						case RIGHT :
							if (defaults.swipeRight)
								defaults.swipeRight.call($this,event, direction, distance, duration);
							break;

						case UP :
							if (defaults.swipeUp)
								defaults.swipeUp.call($this,event, direction, distance, duration);
							break;
						
						case DOWN :	
							if (defaults.swipeDown)
								defaults.swipeDown.call($this,event, direction, distance, duration);
							break;
					}
				}
			}
			...
			*/
			function validateSwipeTime()
			{
				...
				if(defaults.maxTimeThreshold)
				{
					if(duration >= defaults.maxTimeThreshold) 
						result = false;
					else
						result = true;
				}
